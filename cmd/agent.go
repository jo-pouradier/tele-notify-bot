...
func init() {
	...
	agentCmd.Flags().String("name", "", "Name of the agent for for better recognition, if empty it will be a UUID")
	agentCmd.Flags().String("addr", "localhost:50051", "The address to connect to like hostname:port")
	agentCmd.Flags().Bool("tls", false, "Connection uses TLS if true, else plain TCP")
	agentCmd.Flags().String("ca_file", "./x509/ca_cert.pem", "The file containing the CA root cert file")
	agentCmd.Flags().String("server_host_override", "test.tbot.jo-pouradier.fr", "The server name used to verify the hostname returned by the TLS handshake")
}
